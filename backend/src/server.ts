...
app.get('/documentation-pdf', async (req, res, next) => {
    try {
        const rootDir = process.cwd().endsWith('backend') ? path.join(process.cwd(), '..') : process.cwd();
        const tempPath = path.join(rootDir, 'Angular21_Boilerplate_Documentation.pdf');
        await generatePDF(tempPath);
        res.download(tempPath, 'System_Documentation.pdf');
    } catch (err) {
        next(err);
    }
});
